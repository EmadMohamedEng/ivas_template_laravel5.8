                                    ...
                                    <tr>
                                        ...
                                        <td class="visible-md visible-lg">
                                            <div class="btn-group">
                                                ...
                                                <form action="{{ route('provider.destroy' , $value->id )}}"
                                                    method="POST" style="display: initial;">
                                                    @csrf
                                                    <input type="hidden" name="_method" value="DELETE" />
                                                    <button type="submit" class="btn btn-danger" style="height: 28px;"><i
                                                            class="fa fa-trash"></i></button>
                                                </form>
                                                @if(count($value->$categorys) > 0)
                                                <a class="btn btn-sm show-tooltip" title="Show Content" href="{{url('provider/$value->id')}}" data-original-title="show categorys"><i class="fa fa-step-forward"></i></a>
                                                @endif
                                            </div>
                                        </td>
                                    </tr>
